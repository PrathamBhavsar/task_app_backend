...
export const authController = new Elysia({ prefix: '/auth', name: 'Auth', detail: { tags: ['Auth'] } })
    ...
    }, { body: AuthModel.login, detail: { tags: ['Auth'] } })
    .post('/register', async ({ body, set }) => {
        set.status = 201
        const result = await AuthService.register(body)
        return successResponse(result, 'Registration successful')
    }, { body: AuthModel.register, detail: { tags: ['Auth'] } })
    ...
    }, { body: AuthModel.refreshToken, detail: { tags: ['Auth'] } })
